...
%DIF DEL example.tex       Mon Apr 29 15:49:44 2019
%DIF ADD example_new.tex   Mon Apr 29 15:38:57 2019
...
%DIF 15c15
%DIF < \usepackage[inline]{enumitem} % User control over the layout of lists (itemize, enumerate, description).
%DIF -------
%\usepackage[inline]{enumitem} % User control over the layout of lists (itemize, enumerate, description). %DIF > 
%DIF -------
...
\usepgfplotslibrary{statistics} %DIF > 
 %DIF > 
%DIF -------
...
%DIF 112a116-122
\newcounter{tmp@cnt} %DIF > 
 %DIF > 
\newcommand*\combine[1][2]{% %DIF > 
	\refstepcounter{enumi} %DIF > 
	\setcounter{tmp@cnt}{\value{enumi}} %DIF > 
	\addtocounter{enumi}{#1-1} %DIF > 
	\item[\thetmp@cnt--\theenumi\@labelpunc]} %DIF > 
%DIF -------
...
{
	name={job},
	description={Definition of an execution on a back end, including the input data}
}
...
\subsection{\DIFaddbegin \DIFadd{Use Case 1 – }\DIFaddend Re-use of input data}\label{UseCase1}
\begin{figure}[h]
	\centering
...
\subsection{\DIFaddbegin \DIFadd{Use Case 2 – }\DIFaddend Providing job execution Information}\label{UseCase2}
\begin{figure}[h]
	\centering
...
\subsection{\DIFaddbegin \DIFadd{Use Case 3 – }\DIFaddend Compare different job executions}\label{UseCase3}
\begin{figure}[h]
	\centering
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \todo{Add Evaluation Link to this}
...
To make the whole workflow reproducible, every single component \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend to be identifiable. Hence, the following elements get introduced as \DIFdelbegin \DIFdel{additions }\DIFdelend \DIFaddbegin \DIFadd{additional components }\DIFaddend to the back end.
...
The input data of the processing is crucial for the outcome of the job execution. Even though the process graph already contains an identifier of the input data, unique for the specific back end, internal changes to the data set might not result in a new identifier. So jobs called later in time might use another version of the input data than \DIFdelbegin \DIFdel{the }\DIFdelend previous jobs. To prevent this the input data has to be persisted according to the 14 steps of data provenance defined by the RDA \cite{rauber2016identification}. Every back end is responsible for applying data identification. It is assumed that the back ends preserve the data as described in Section \DIFdelbegin \DIFdel{\ref{Design:Data Identification}}\DIFdelend \DIFaddbegin \DIFadd{\ref{Data Identification}}\DIFaddend . The Query Handler is the module where the data persistence is implemented and depends highly on the structure and architecture of the back end. Therefore, no common definitions can be made for all back ends in this section. In Chapter \ref{Implementation} there is a fully implemented solution at the EODC back end. The \DIFdelbegin \DIFdel{context model elements of the job executions are specified with a "J" in the beginning for readability reasons. Elements of context model related to the back end environment are specified with a "B" in the beginning. Each element represents one data entry of the resulting context model. The }\DIFdelend input data is represented in the context model by the following element: 
\DIFaddbegin 

\begin{enumerate}[(a)]
\item \textbf{\DIFadd{Input data persistent identifier}} \DIFaddend \\
	\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdel{J1: Input data persistent identifier}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend The output of the Query Handler is the PID of the input data related to a job execution. Every job execution uses input data, which has a PID assigned to it. Therefore, the PID is added to the job dependent context model. 
...
The scope of this part of the context model is to get the static environment of where the job execution at the back end takes place. It contains the provenance data that is independent of a job execution, so it does not change through job executions, regardless of how many jobs were processed. It can only change from inside of the back end by their maintainers. The earth observation community has a great variety of different back end providers with very distinct setups. The challenge of this part of the design is to make it simple and generic. Data captured in this part of the context model is not meant to be shown to the user directly, because of security issues. The \DIFdelbegin \DIFdel{context model elements of the back end environment are specified with a "B" in the beginning for readability reasons. Elements of the job dependent context model are specified with a "J" in the beginning. The }\DIFdelend following meta-data is suggested for the back end provenance:
\DIFaddbegin 

\begin{enumerate}[(A)]
	\item \textbf{\DIFadd{Code identification}} \DIFaddend \\
	\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdel{B1: Code identification}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend Every earth observation back end has to provide an identifier for the used code. Therefore, a version control system has to be applied to the running services. If \DIFaddbegin \DIFadd{the }\DIFaddend back end is open source, it can provide a GitHub repository were the used code is stored and publicly available. The aim of this strategy is to get other back ends with similar settings to reuse the already working setups of the running back ends. In this case the information is added to the back end provenance by saving the git repository URL, the used commit identifier and local changes to the repository. If the back end is not open source\DIFaddbegin \DIFadd{, }\DIFaddend a local or secure version control repository has to be implemented to keep track of the different code versions. %DIF < \\ \\
%DIF < \textbf{B2: Used Folders} \\
%DIF < The back end will most probably not only use the GitHub repository for the processing, so that other folders are involved. For example configuration files may not be persisted in GitHub for security reasons. This information is also crucial for the job execution and therefore needs to be stored. Changes of this directory have to be detected and stored in the context model. Therefore, a checksum or hash key of the additional folders are created and persisted in the context model.     
%DIF < \\ \\
%DIF < \textbf{B3: OS and packages} \\
%DIF < The operating system can also have an impact on the processing and so it has to be stored to the context model too. Not only the information about the operating system, but also the installed packages are added to the context model. If the whole processing is done in a virtual container the operating system of that container shall be stored. If the whole processing is done in a docker container, the docker description file can be used for the environment data. 
\DIFaddbegin \DIFadd{The code identifier is necessary to identify differences in the back end code and the version control system enables to jump back to versions of interest. 
	}\item \textbf{\DIFadd{API version}} \DIFaddend \\
	\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdel{B2: API version}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{The back ends }\DIFdelend \DIFaddbegin \DIFadd{The back end }\DIFaddend API version is the version of the interface API it currently \DIFdelbegin \DIFdel{is using. The }\DIFdelend \DIFaddbegin \DIFadd{uses. The API defines the syntax and semantic of the communication. The }\DIFaddend same API calls can resolve in different results if the version of it differs \DIFaddbegin \DIFadd{from the original execution}\DIFaddend , hence the API version is persisted in the context model.
	\DIFaddbegin \item \textbf{\DIFadd{Back end version}} \DIFaddend \\
	\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdel{B3: Back end version}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend The back end version is an identifier that gets updated on every change of the back end. This includes changes on the hardware and software, so that every change on the code identifier or the API version results in a new back end version. \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend To provide long term stability of the capturing, it is essential to implement a tool to automatically capture the environment data and persist it in a separate back end version store. Every change on any of the \DIFdelbegin \DIFdel{previously }\DIFdelend described back end meta-data has to be detected automatically and have to result in a new back end version.    
	\DIFaddbegin \item \textbf{\DIFadd{Publication time-stamp}} \DIFaddend \\
	\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdel{B4: Publication time-stamp}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend The time-stamp of when the version was accessible at the back end. The time-stamp stands for the beginning of \DIFdelbegin \DIFdel{the version of the back end }\DIFdelend \DIFaddbegin \DIFadd{a back end version}\DIFaddend . If there exists no time-stamp after that, then it is the current used version. This enables the possibility to find the version of the back end used by a job executed in the past, by only knowing the time it was executed.     
\DIFaddbegin \end{enumerate}
\DIFaddend 
...
The way of executing a specific process graph is not only related to the code running it, but also by the dependencies of the code. Therefore, the programming language version and the additional used libraries of it are persisted in the context model. To gain \DIFdelbegin \DIFdel{more }\DIFdelend meta-information about the processing the start time and end time can be added to the context model via timestamps. This leads to the following capturing elements in the job context model.

\DIFaddbegin \begin{enumerate}[(a)]
	\setcounter{enumi}{2}
	\item \DIFaddend \textbf{\DIFdelbegin \DIFdel{J3: }\DIFdelend Programming language}\\
...
\DIFdelbegin \subsubsection{\DIFdel{Back end provenance}}%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{Job:Back end provenance}
%DIFDELCMD < %%%
\DIFdel{The back end provenance described in Section \ref{Design:Back end provenance} defines the version of the code that is executed and therefore has to be added to the job context model. Over time the back end provenance will get updated so that every job need to persist a reference to the original back end configuration at the execution time. There are two solutions to achieve this. First the identifier of a specific version of the back end gets added to the context model. In this case the data of the back end environment versions have to be persisted and accessible. The second solution is to put a time-stamp of the execution into the job execution context model.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{J2: Back end provenance / code identifier}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{The version of the back end provenance during the job execution. Since for every change on the back end a new version will be applied, the version of the back end is used as a code identifier for the execution. Another solution is to persist an execution time-stamp, so that the version of the given time can be identified.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Result Handler}\label{Design:Result Handler}
...
\DIFaddbegin \begin{enumerate}[(f)]
	\item \DIFaddend \textbf{\DIFdelbegin \DIFdel{J6: }\DIFdelend Result hash} \\
...
The context model is the data record that saves the provenance of a job execution. The type of storage it is persisted in, is defined by the infrastructure of the back end provider. It can \DIFaddbegin \DIFadd{e.g. }\DIFaddend be stored in a relational database or in a file-based system as a file\DIFdelbegin \DIFdel{on the file system}\DIFdelend . It has to be \DIFdelbegin \DIFdel{added to the meta-data database }\DIFdelend \DIFaddbegin \DIFadd{integrated into the meta-database }\DIFaddend of the back end. The back end has to persist two different types of datasets\DIFdelbegin \DIFdel{: }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{First the back end }\DIFdelend \DIFaddbegin \DIFadd{. Figure \ref{fig:design_contextmodel} shows an overview of the elements of the back end used to run a job. In addition it shows how the context model elements are used to identify the components of the back end.
}

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{design_context_model}
	\caption{\DIFaddFL{Overview of the back end execution components and the context model elements that identifies them.}}
	\label{fig:design_contextmodel} 
\end{figure}

\DIFadd{The back end }\DIFaddend environment dataset that saves job independent provenance and results in a back end version. The mandatory element of the back end environment is the back end version that defines the state of the back end in a specific time. The back end version need to be resolvable by a specific code version that was present in a past execution. In addition meta-data about the back end can be added. The elements (\DIFdelbegin \DIFdel{B1-B4) }\DIFdelend \DIFaddbegin \DIFadd{(A) - (D)) }\DIFaddend defined in Section \ref{Design:Back end provenance} are the suggestions of elements in the back end environment \DIFdelbegin \DIFdel{context model. }\DIFdelend \DIFaddbegin \DIFadd{record. The following list recaps all of the back end environment elements: 
}

\begin{enumerate}[(A)]
	\item \textbf{\DIFadd{Code identification}} \DIFaddend \\
	\DIFdelbegin \DIFdel{Second the }\DIFdelend \DIFaddbegin \DIFadd{Enables the identification of the code of a back end version.
	}\item \textbf{\DIFadd{API version}} \\
	\DIFadd{Enables the identification of the API version of a back end version.
	}\item \textbf{\DIFadd{Back end version}} \\ 
	\DIFadd{Identifies the whole back end.
	}\item \textbf{\DIFadd{Publication time-stamp}} \\ 
	\DIFadd{Enables the identification of a back end version at a specific time.
}\end{enumerate}

\DIFadd{The }\DIFaddend job execution context model that includes all information that is dedicated to a specific job is persisted in the context model. There are three mandatory elements in the job context model. The input data identifier has to be persisted in the context model, so that it can be identified and used in future job executions. The used version of the back end related to the execution has to be included in the context model, to be able to re-execute it with the same code in the future. The output hash has to be added to the job dependent context model, so that different results can be detected. \DIFdelbegin \DIFdel{In Section \ref{Design} the elements (J1-J6) are additional jobspecific meta-data for the context model.  
}\DIFdelend \DIFaddbegin \DIFadd{The following list recaps the elements of the job context model described in Section \ref{Design:Job dependent provenance}: 
}\DIFaddend 

%DIF < \textbf{J2: Process Code Identifier}
%DIF < The executed  code for the process has to be identifiable, therefore there has to be an id of the code. The OpenEO project is an OpenSource project and the back end provider have the used code at the Github repository, so the link to the github repository with the currently checked out commit can be used as the identifier. 
\DIFaddbegin \begin{enumerate}[(a)]
	\item \textbf{\DIFadd{Input data persistent identifier}} \\
	\DIFadd{Identifies the input data of the job.
	}\item \textbf{\DIFadd{Back end provenance / code identifier}} \\
	\DIFadd{Identifies the back end version (C) during the execution of the job. Connects the job context model with the back end environment dataset.
	}\item \textbf{\DIFadd{Programming language}} \\
	\DIFadd{Identifies the programming language and version used by the job.
	}\item \textbf{\DIFadd{Dependencies of the programming language}} \\
	\DIFadd{Identifies the dependencies of the programming language used by the job.
	}\item \textbf{\DIFadd{Start and end time of process execution}} \\
	\DIFadd{Provides the start and end time of the job execution.
	}\item \textbf{\DIFadd{Result hash}} \\
	\DIFadd{Describes the job results for comparing it with other job results.
}\end{enumerate}
\DIFaddend 
...
Provenance information need be provided to the user. Therefore, there have to be additions to the core API specification. Additional endpoints for the users to get information about the back end have to be created for the core API, back end and client. The following recommendations \DIFdelbegin \DIFdel{summarize the set of needed additions, they are structured with a "U" in the beginning to add readability through the thesis. Other than the "J" and the "B" data elements, the following points are implementation recommendations }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend for back end \DIFaddbegin \DIFadd{and client }\DIFaddend provider to make \DIFdelbegin \DIFdel{the use of the }\DIFdelend \DIFaddbegin \DIFadd{features related to the }\DIFaddend context model accessible for the users.

\DIFaddbegin \begin{enumerate}[I.]
\item \DIFaddend \textbf{\DIFdelbegin \DIFdel{U1: }\DIFdelend Back end version} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{U2: }\DIFdelend Detailed Job Information} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{U3: }\DIFdelend Comparing Two Jobs} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{U4: }\DIFdelend Data Identifier Landing Page} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{U5: }\DIFdelend Re-use of Input Data} \\
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{enumerate}[(A)]
\item \DIFaddend \textbf{\DIFdelbegin \DIFdel{B1: }\DIFdelend GitHub Repository} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{B2: }\DIFdelend Core API Version} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{B3: }\DIFdelend Back End Version} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{B4: }\DIFdelend Publication Time-stamp} \\
...
\begin{table}[]
	...
	\begin{tabular}{l|l}
		...
		\textbf{\DIFdelbeginFL \DIFdelFL{J1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(a)}\DIFaddendFL : Input data persistent identifier} & input\_data \\ \hline
		\textbf{\DIFdelbeginFL \DIFdelFL{J2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(b)}\DIFaddendFL : Back end provenance / code identifier} & backend\_env \\ \hline
		\textbf{\DIFdelbeginFL \DIFdelFL{J3}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(c)}\DIFaddendFL : Programming language} & interpreter \\ \hline
		\textbf{\DIFdelbeginFL \DIFdelFL{J4}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(d)}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{Installed packages }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Dependencies }\DIFaddendFL of the programming language} & code\_env \\ \hline
		\textbf{\DIFdelbeginFL \DIFdelFL{J5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(e)}\DIFaddendFL : Start and end time of the process execution} & start\_time, end\_time \\ \hline
		\textbf{\DIFdelbeginFL \DIFdelFL{J6}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(f)}\DIFaddendFL : Result hash} & output\_data \\ %\hline
...
\begin{enumerate}
	\item \textbf{Process Execution} \\
	The \textit{Process Execution} module at the EODC back end is responsible for the actual execution of the job. It creates the additional logging information (\DIFdelbegin \DIFdel{J1, J3, J4 and J5) }\DIFdelend \DIFaddbegin \DIFadd{(a), (c), (d) and (e) of the context model) }\DIFaddend and persists it in the logging file of the job execution. 
	...
	\item \textbf{Job Capturing} \\
	The \textit{Job Capturing} module parses the logging file to extract the information needed for the context model (\DIFdelbegin \DIFdel{J1, J3, J4 and J5)}\DIFdelend \DIFaddbegin \DIFadd{(a), (c), (d) and (e)) }\DIFaddend and passes the formatted information to the \textit{Context Model Creator}. The \textit{Context Model Creator} needs the \textit{Result Handler} (\DIFdelbegin \DIFdel{J6}\DIFdelend \DIFaddbegin \DIFadd{f}\DIFaddend ) and \textit{Back End Provenance Handler} (\DIFdelbegin \DIFdel{J2}\DIFdelend \DIFaddbegin \DIFadd{b}\DIFaddend ) for the remaining parts of the context model. After receiving all necessary information the JSON context model is created and stored to the Job table in the EODC meta-database.    
\end{enumerate}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{enumerate}[(a)]
\item \DIFaddend \textbf{\DIFdelbegin \DIFdel{J1:  }\DIFdelend Source Input Data Identifier} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{J2: }\DIFdelend Back end provenance / Code Identifier} \\
...
\DIFaddbegin \item[\DIFadd{(c)(d)}] \DIFaddend \textbf{\DIFdelbegin \DIFdel{J3 and J4: }\DIFdelend Programming language and  installed packages of programming language} \\
...
\DIFaddbegin \item[\DIFadd{(e)}] \DIFaddend \textbf{\DIFdelbegin \DIFdel{J5: }\DIFdelend Start and end time of process execution} \\
...
\DIFaddbegin \item[\DIFadd{(f)}] \DIFaddend \textbf{\DIFdelbegin \DIFdel{J6: }\DIFdelend Result Identifier} \\
...
\DIFaddend \section{User Services}\label{Implementation:User Interface}
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{Implementation of U2: }\DIFdelend Detailed Job Information} \\
...
\DIFaddbegin \item \DIFaddend \textbf{\DIFdelbegin \DIFdel{Implementation of U3: }\DIFdelend Comparing two Jobs} \\
...
{
"process_graph":"EQUAL",
"\DIFdelbegin \DIFdel{interpreter":"EQUAL",
"}\DIFdelend input_data":"EQUAL",
"code_env":"EQUAL",
"output_data":"EQUAL",
"openeo_api":"EQUAL",
"different":{
   "\DIFaddbegin \DIFadd{interpreter":"Python 3.5",
   "}\DIFaddend job_id":"jb-47e062e4-d39c-4f7f-bc5e-aa877f039a84",
...
This section shows how the use cases of Section \ref{Use Cases} can be addressed using the implementation described in the sections before. \DIFaddbegin \DIFadd{In the first section there is an overview how the data identification recommendations by \acrshort{rda} are implemented. 
}\DIFaddend 

\subsection{\DIFaddbegin \DIFadd{Use Case 1 – }\DIFaddend Re-Use of Input Data}\label{Implementation:Use Case1}
...
\subsection{\DIFaddbegin \DIFadd{Use Case 2 – }\DIFaddend Capturing job dependent environments}\label{Implementation:Use Case2}
This scenario focuses on the execution environment. It handles the need of researchers to describe the execution process. \DIFaddbegin \DIFadd{Section \ref{UseCase2} describes the second use case in more detail. }\DIFaddend The implementation at the EODC back end gives the users the option to gain additional meta-data about the execution. In the following the steps of the scenario are run with the implemented instance of the EODC back end.    
...
\end{code}
	After the execution of the lines above, the researcher is able to get the used programming language including the version and installed packages. In addition the back end version describes the used processing code. For more transparency the researcher can link to the used GitHub repository and the commit \DIFdelbegin \DIFdel{used for the experiment. Figure \ref{fig:usecase2_2} }\DIFdelend \DIFaddbegin \DIFadd{to identify the used code. Listing \ref{lst:use_case2_logfile} }\DIFaddend shows the output of the logging calls. 

\end{enumerate}

\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{usecase2_2}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \begin{code}
	\begin{minted}[fontsize=\footnotesize]{text}
\DIFaddFL{INFO:root:Interpreter: Python 3.7.1
INFO:root:Code Environment: }[\DIFaddFL{'alembic==0.9.9', 'amqp==1.4.9',
         ..., 'GitPython==2.1.11', 'numpy==1.16.2', 'GDAL==2.4.0'}]
\DIFaddFL{INFO:root:Input Data PID URL:
...
INFO:root:Back End Version (commit): 
	...
	\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Screenshot of logging }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Logging }\DIFaddendFL output of the second Use Case.}
	\DIFdelbeginFL %DIFDELCMD < \label{fig:usecase2_2} %%%
%DIF <  \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{lst:use_case2_logfile}
\end{code}
%DIF > \begin{figure}[h]
%DIF > 	\centering
%DIF > 	\includegraphics[width=\textwidth]{usecase2_2}
%DIF > 	\caption{Screenshot of logging output of the second Use Case.}
%DIF > 	\label{fig:usecase2_2} % \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
%DIF > \end{figure}
\DIFaddend 

\subsection{\DIFaddbegin \DIFadd{Use Case 3 – }\DIFaddend Getting differences of job executions}\label{Implementation:Use Case3}
The last use case is focused on the users of OpenEO. It describes the need of transparency of job executions for the users. If results differ with the same job later in time, the user can access meta-data to find reasons why it happened. Other users might compare different job execution environments. \DIFaddbegin \DIFadd{Section \ref{UseCase3} describes the third use case in more detail.  
...
logging.info(diffAC)
	...
	With the lines above the researcher gets two dictionaries of the comparisons between job A with job B (diffAB) and job A with job C (diffAC). The content of the dictionary is a comparison of every key of the jobs context model with each other. \DIFdelbegin \DIFdel{Figure \ref{fig:eva_diff} is a screenshot of the logging messages defined in }\DIFdelend \DIFaddbegin \DIFadd{Listing \ref{lst:use_case3_logfile} shows the logging output of }\DIFaddend Listing \ref{lst:impl_usecase3_4}.
\end{enumerate}

\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{usecase3_4}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > \begin{figure}[h]
%DIF > 	\centering
%DIF > 	\includegraphics[width=\textwidth]{usecase3_4}
%DIF > 	\caption{Logging output of the job comparisons diffAB and diffAC.}
%DIF > 	\label{fig:eva_diff} % \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
%DIF > \end{figure}
\DIFaddbeginFL 

\begin{code}
	\begin{minted}[fontsize=\footnotesize]{text}
\DIFaddFL{INFO:root:}{\DIFaddFL{'input_data': 'EQUAL', 'output_data': 'EQUAL', 
 'process_graph': 'EQUAL', 'openeo_api': 'EQUAL', 
 'interpreter': 'EQUAL', 'code_env': 'EQUAL',
 'different': 
 }{\DIFaddFL{'back_end_timestamp': '20190417194702.496810', 
...
\DIFaddFL{INFO:root:}{\DIFaddFL{'input_data': 'EQUAL', 'output_data': 'EQUAL', 
 ...
 'different': }{\DIFaddFL{'process_graph': }{\DIFaddFL{'imagery': }{\DIFaddFL{'imagery': }{\DIFaddFL{'extent': 
 }{\DIFaddFL{'crs': 'EPSG:4326', 'east': 12.189331, 'north': 46.905246, 
 'south': 45.935871, 'west': 10.288696}}\DIFaddFL{, 'imagery': 
 }{\DIFaddFL{'extent': }[\DIFaddFL{'2017-05-01', '2017-05-31'}]\DIFaddFL{, 
 'imagery': }{\DIFaddFL{'name': 's2a_prd_msil1c', 'process_id': 'get_collection'}}\DIFaddFL{, 
 'process_id': 'filter_daterange'}}\DIFaddFL{, 'process_id': 'filter_bbox'}}\DIFaddFL{, 
 'nir': 'B08', 'process_id': 'NDVI', 'red': 'B04'}}\DIFaddFL{, 'process_id': 
 'max_time'}}\DIFaddFL{, 'start_time': '2019-04-17 19:47:09.089075', 
 ...
 'job_id': 'jb-ecdd5768-3c22-4c73-85b8-ac6f4bdc138f'}}}
	\end{minted}
	\DIFaddendFL \caption{Logging output of the job comparisons diffAB and diffAC.}
	\DIFdelbeginFL %DIFDELCMD < \label{fig:eva_diff} %%%
%DIF <  \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{lst:use_case3_logfile}
\end{code}
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo{optional: Maybe adding a sequence diagram somewhere in the implementation showing how things work would complement it?}
\DIFaddbeginFL 
...
\begin{itemize}
	...
	The duration of the result hash creation is dependent on the length of the resulting file list. The sha-256 operation has a complexity of $O(n)$, where n is the length of the resulting file list string. In the test cases the duration time of the result hash calculation is between 28 µs and 51 µs with a median of 9167 µs. Figure \ref{fig:evaluation_data_resulthash} shows the duration of the result hash creation of the test cases, sorted by the size of the result file set in an ascending way. The data of the chart is in Table \ref{Tab:data_result_hash}, which shows the relationship between number of files and average duration time of the test cases.  
...
Figure \ref{fig:eva_data_static} show the duration of the static query entry elements of the different test cases. The test cases are sorted by the result size in ascending order.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.55]{eva_data_static}
	\caption{Static elements of the query table in relation to the test cases sorted by resulting size.}
...
%\todo{ What is the variance across several runs?}
%\todo{Context Model: mock-up of result hash calculation possible ?}
...
\section{Summary}
The evaluation in this chapter showed how the solution tackles the goals of the research questions. First it summarizes how the RDA recommendations are implemented in the solution, except for the migration recommendations (R14 and R15). The data identification implementation is then tested against special test cases regarding data updates and data deletions at the back end. The evaluation shows that the solution can re-execute queries properly by returning old versions of updated files. The test cases show that the usage of the data pid as input data of a new job is superior to the current way of re-executing a job with the same process graph at EODC. The reason for this is, that the process graph does not have the original execution time-stamp and therefore does not use the same input data after an update occurs. In the evaluation of deleted data at the back end, the solution happens to be not capable of showing the exact missing files, since not the whole file list result of the query is persisted. The test case on job capturing showed that the solution is capable of identifying the back end version and therefore, the environment of the job execution. Still to run a new job on the same environment, EODC has to manually provide it. The evaluation also contains a section about the performance and storage impact on the EODC back end, by running 18 test cases derived from past publications that used data from EODC. The results show that, except for the result hashes, the calculation of the data identification and the context model are independent from the complexity of the job. The result hash of the data identification is dependent on the size of the query result and the result hash of the context model is dependent on the size of the output file of the job execution. The storage impact evaluation results in the conclusion that the additional needed space per job is static. It depends on if there is a new query entry added to the database or not.     
...
%DIF < \chapter{Appendix}\label{Appendix}
\DIFaddbegin \chapter{\DIFadd{Appendix}}\label{Appendix}
\DIFaddend 

%DIF < \section{Papers for Evaluation}
\DIFaddbegin \begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{appendix_pidB}
	\caption{\DIFaddFL{Original query of jobB from the evaluation in Section \ref{Evaluation:special_dataid}.}}
	\label{fig:appendix_pidB} %DIF >  \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{appendix_pidD}
	\caption{\DIFaddFL{Original query of jobD from the evaluation in Section \ref{Evaluation:special_dataid}.}}
	\label{fig:appendix_pidD} %DIF >  \label has to be placed AFTER \caption (or \subcaption) to produce correct cross-references.
\end{figure}
\DIFaddend 

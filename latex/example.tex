...
\todo{Read through RDA mapping --> maybe move to Implementation ?}

\section{Special Test Cases}\label{Evaluation:special}
\todo{Change to first text then code: Always start a description explaining what’s the goal of this test and why you show it. Readers are not going to guess.}
...
Table \ref{Tab:eva_datachanges1} shows the content of the query table after the execution of this step. There is one entry with a new generated query pid, by the Query Handler, since there was no query entry in the database that matches the normalized hash and the result hash. 
	...
	\begin{table}[]
		...
		\centering
		\begin{tabular}{c|l}
			\textbf{Column} & \textbf{Entry 1} \\ \hline
			...
			created\_at & 2019-03-31 17:36:43.064445   \\
		\end{tabular}
		\label{Tab:eva_datachanges1}
	\end{table}
\end{enumerate}

\subsubsection{Test Case 1: Is it possible to re-execute a query after a data set is updated?}
%{ \large \textbf{} } \\
...
(file_listA == file_listC) # Returns True
		\end{minted}
		\caption{Step 5: Create job C, which uses the input data identified by pidA.}
		\label{lst:eva_datachange_5}
	\end{code}
\end{enumerate}

\todo{Restructure to answer the questions so that everyone gets it -.-}
\todo{See E-Mail for further questions e.g. EODC policy}

{ \large \textbf{Is it possible to re-execute a query after a data set is updated with the original one deleted?}} \\
\begin{enumerate}[2]
...
\todo{Add message, which shows the files that differ}
\begin{figure}[h]
	\centering
...
{ \large \textbf{Is it possible to re-execute a query after a data set is deleted?}} \\
...
\begin{table}[]
	...
	\begin{tabular}{r|r|r|r|r|r}
		...
		\textbf{Test} & \textbf{Ref. Back end} & \multicolumn{3}{c}{\textbf{Solution Back end}} &  \\ \textbf{Case} & \textbf{Total} & \textbf{Total} & \textbf{Query Handler} & \textbf{Context Model} & \textbf{Result Files} \\ \hline
		1 & 800.136 ms & 831.136 ms & 14.318 ms & 16.682 ms & 12 \\ 
		2 & 284.361 ms & 335.556 ms & 24.167 ms & 27.028 ms & 16 \\ 
		3 & 307.439 ms & 359.268 ms & 22.951 ms & 28.878 ms & 4 \\ 
		4 & 266.351 ms & 309.541 ms & 20.351 ms & 22.838 ms & 2 \\ 
		5 & 284.324 ms & 345.757 ms & 26.081 ms & 35.351 ms & 5 \\ 
		6 & 289.622 ms & 347.578 ms & 24.956 ms & 33.000 ms & 3 \\ 
		7 & 270.889 ms & 327.289 ms & 25.533 ms & 30.867 ms & 3 \\ 
		8 & 278.091 ms & 329.409 ms & 22.909 ms & 28.409 ms & 6 \\ 
		9 & 13296.605 ms & 13604.698 ms & 259.884 ms & 48.209 ms & 2443 \\ 
		10 & 386.684 ms & 435.895 ms & 22.289 ms & 26.921 ms & 8 \\ 
		11 & 40512.538 ms & 40770.026 ms & 212.667 ms & 44.821 ms & 2302 \\ 
		12 & 528.450 ms & 566.925 ms & 18.675 ms & 19.800 ms & 6 \\ 
		13 & 1785.462 ms & 1818.641 ms & 18.410 ms & 14.769 ms & 57 \\ 
		14 & 40461.526 ms & 40729.368 ms & 223.000 ms & 44.842 ms & 2403 \\ 
		15 & 422.512 ms & 466.854 ms & 20.146 ms & 24.195 ms & 10 \\ 
		16 & 44672.378 ms & 44940.022 ms & 221.889 ms & 45.756 ms & 2435 \\ 
		17 & 753.256 ms & 787.436 ms & 15.897 ms & 18.282 ms & 51 \\ 
		18 & 281.611 ms & 344.389 ms & 26.278 ms & 36.500 ms & 157 \\ \hline
		\textbf{Avg.} & \textbf{8104.569 ms} & \textbf{8202.766 ms} & \textbf{67.800 ms} & \textbf{30.397 ms} & \textbf{507.333} \\ 
	\end{tabular}
...
\todo{Show the whole context model and what storage is needed to capture the single parts}
\section{Summary}

...
\begin{table}[]
	...
	\begin{tabular}{r|r|r|r|r|r}
		...
		\textbf{Test} & \textbf{Ref. Back end} & \multicolumn{3}{c|}{\textbf{Solution Back end}} &  \\ \textbf{Case} & \textbf{Total} & \textbf{Total} & \textbf{Query Handler} & \textbf{Context Model} & \textbf{Comparison} \\ \hline
...
\begin{itemize}
	...
	The dataset\_pid is taken directly from the parsed filter arguments of EODC and has a constant duration time. In the test cases the duration of the dataset\_pid is between 4 ms and 8 ms with a median of 8 ms. 
	\item \textbf{original} \\
	The original query is passed by the Query Execution component of EODC. The Query Handler transforms the query into a string, which takes constant execution time between 24 ms and 98 ms with a median of 39 ms during the test cases.
	\item \textbf{normalized} \\
	The normalized query is the result of an alphabetical sorting of the parsed filter arguments of EODC. The sorting takes nearly constant duration time, since there are only 4 filter arguments allowed in the OpenEO API version 0.3.1, but they may not all be used. Depending on the amount of used filter arguments, the sorting algorithm need a different amount of iterations and computation time. In this evaluation all possible filter arguments were used for each test case, therefore it has a constant duration time. In the test cases it takes between 38 ms and 132 ms with a median of 61 ms.  
	\item \textbf{norm\_hash} \\
	The performance of the normalized query hash is dependent on the number of filter arguments of the normalized query. In this evaluation it is constant, since in every test case, there are all filter arguments used. Hence, the evaluation needs the upper limit of the possible duration time. In the test cases the the duration is between 15 ms and 49 ms with a median of 21 ms.  
	\item \textbf{result\_hash} \\
	The duration of the result hash creation is dependent on the length of the resulting file list. In the test cases the duration time of the result hash calculation is between 18 ms and 232 ms with a median of 39 ms. 
	\todo{make analysis on result files}  
	\item \textbf{meta\_data} \\
	The meta data consists only on the number of result files. This is measured by the build in len() operator in python, which has a static performance independent on the length of a list according to the official python wiki\footnote{https://wiki.python.org/moin/TimeComplexity}. In the test cases the meta data calculation takes a duration between 6 ms and 38 ms with a median of 12 ms.
	\item \textbf{Check on existing query record} \\
	To ensure that there are no duplicate query entries in the database, the Query Handler checks if there is already an entry with the same normalized query hash and the same result hash. It depends on the amount of query entries in the database. The database query in the test cases takes between 1488 ms and 3078 ms with a median of 2205 ms. 
	\todo{get reference on how fast a select statement is in PostGreSQL}  
	\item \textbf{Persisting new query record} \\
	The query\_pid is generated by the python uuid module, automatically by inserting the query entry into the database. Since the query entries are almost constant in size (see Section \ref{Evaluation:impact_stor}), the duration is constant. The duration of storing the query entry in the test cases is between 3013 ms and 16.756 ms with a median of 4102.5 ms.  
\end{itemize}

...
\todo{Add Evaluation Link to this}
...
\end{enumerate}
\todo{Maybe add summary of changes to the EODC back end with a "killer" diagram}
\section{Use Cases}
...
\subsection{Data Identification}\label{Evaluation:dataidentification}
...
\begin{itemize}
	\item \textbf{R1: Data Versioning} \\
	Persistence of old data versions may not be applied, due to the size of the data. The versioning of data is implemented by EODC. The path to the files are the version identifier, since modified files must have a different file name.
	\item \textbf{R2: Timestamping} \\
	The creation time-stamp of the files are stored by the file system of EODC, since the files have to change the name after updates, the creation time-stamp of the updated file is the update time-stamp of the original file. 
	\item \textbf{R3: Query Store Facilities} \\
...
\todo{optional: Maybe adding a sequence diagram somewhere in the implementation showing how things work would complement it?}
...
\todo{See E-Mail for further questions e.g. EODC policy}
\todo{Update the statement about EODC according to the E-Mail of Luca Foresta}
...
\subsection*{Test Case 1: Is it possible to re-execute a query after a data file is updated?}	\label{Tab:eva_datachanges_tc1}
%{ \large \textbf{} } \\
...
\subsection*{Test Case 2: Is it possible to re-execute a query after a data file is updated with the original one deleted?}
...
\begin{table}[]
	...
	\begin{tabular}{r|r|r|r|r|r}
		...
		\textbf{} & \textbf{Ref. Back end} & \multicolumn{3}{c|}{\textbf{Solution Back end}} &  \\ \hline \textbf{Test} & \textbf{} & \textbf{} & \textbf{Query} & \textbf{Context} & \textbf{Back end} \\ 
		\textbf{Case} & \textbf{Total} & \textbf{Total} & \textbf{Handler} & \textbf{Model} & \textbf{Comparison} \\ \hline
		1 & 322.946 ms & 343.127 ms & 14.187 ms & 5.994 ms & 20.181 ms \\ 
		2 & 369.066 ms & 391.505 ms & 16.342 ms & 6.097 ms & 22.439 ms \\ 
		3 & 281.657 ms & 303.406 ms & 15.669 ms & 6.081 ms & 21.749 ms \\ 
		4 & 276.324 ms & 295.954 ms & 14.015 ms & 5.615 ms & 19.630 ms \\ 
		5 & 312.150 ms & 331.802 ms & 13.925 ms & 5.727 ms & 19.652 ms \\ 
		6 & 314.571 ms & 334.290 ms & 13.985 ms & 5.734 ms & 19.720 ms \\ 
		7 & 320.081 ms & 340.553 ms & 14.555 ms & 5.916 ms & 20.471 ms \\ 
		8 & 304.998 ms & 325.633 ms & 14.742 ms & 5.893 ms & 20.635 ms \\ 
		9 & 565.289 ms & 735.751 ms & 148.766 ms & 21.696 ms & 170.462 ms \\ 
		10 & 401.922 ms & 422.825 ms & 14.874 ms & 6.030 ms & 20.904 ms \\ 
		11 & 521.022 ms & 780.185 ms & 240.660 ms & 18.503 ms & 259.163 ms \\ 
		12 & 387.536 ms & 409.079 ms & 15.711 ms & 5.832 ms & 21.543 ms \\ 
		13 & 510.517 ms & 533.784 ms & 17.070 ms & 6.197 ms & 23.267 ms \\ 
		14 & 657.989 ms & 684.329 ms & 19.010 ms & 7.330 ms & 26.340 ms  \\ 
		15 & 345.806 ms & 368.985 ms & 17.027 ms & 6.151 ms & 23.179 ms \\ 
		16 & 585.730 ms & 828.493 ms & 223.956 ms & 18.807 ms & 242.764 ms \\ 
		17 & 563.755 ms & 586.776 ms & 16.778 ms & 6.243 ms & 23.021 ms \\ 
		18 & 836.377 ms & 859.272 ms & 16.801 ms & 6.093 ms & 22.894 ms \\ \hline
		\textbf{Avg.} & \textbf{437.652 ms} & \textbf{493.097 ms} & \textbf{47.115 ms} & \textbf{8.330 ms} & \textbf{55.445 ms} \\ 
	\end{tabular}
	\label{Tab:eva_performance}
\end{table}
\todo{difference not correct ...}
...
There are 3 test cases with a higher complexity then the other ones, therefore the 
...
[
ytick={1,2},
yticklabels={checkexisting small, querypersist},
]

\addplot+[
boxplot prepared={
	lower whisker=11.418,
	lower quartile=22.359,
	median=132.682,
	upper quartile=141.607,
	upper whisker=169.772,
},
] coordinates {};
\addplot+[
boxplot prepared={
	lower whisker=37.711,
	lower quartile=48.413,
	median=52.794,
	upper quartile=57.040,
	upper whisker=85.255,
},
] coordinates {};
\end{axis}
\end{tikzpicture}
...
[
ytick={1,2},
yticklabels={checkexisting, querypersist},
]
\addplot+[
boxplot prepared={
	lower whisker=1.488,
	lower quartile=1.747,
	median=2.093,
...
boxplot prepared={
	lower whisker=3.013,
	lower quartile=3.690,
	median=4.284,
	upper quartile=9.242,
	upper whisker=16.911,
},
] coordinates {};
\end{axis}
\end{tikzpicture}
...
[
ytick={1,2,3,4,5,6},
yticklabels={normalized, normhash, resulthash, datasetpid, origquery, filenumber},
]
\addplot+[
boxplot prepared={
	lower whisker=62,
	lower quartile=374.5,
	median=625,
	upper quartile=852,
	upper whisker=2781,
},
...
\begin{itemize}
	...
	The dataset\_pid is taken directly from the parsed filter arguments of EODC and has a constant duration time. In the test cases the duration of the dataset\_pid is between 4 µs and 32 µs with a median of 8 µs. 

	\item \textbf{original} \\
	The original query is passed by the Query Execution component of EODC. The Query Handler transforms the query into a string, which takes constant execution time between 24 µs and 435 µs with a median of 38 µs during the test cases.
	\item \textbf{normalized} \\
	The normalized query is the result of an alphabetical sorting of the parsed filter arguments of EODC. The sorting takes nearly constant duration time, since there are only 4 filter arguments allowed in the OpenEO API version 0.3.1, but they may not all be used. Depending on the amount of used filter arguments, the sorting algorithm need a different amount of iterations and computation time. In this evaluation all possible filter arguments were used for each test case, therefore it has a constant duration time. In the test cases it takes between 38 µs and 2781 µs with a median of 64 µs.  
	\item \textbf{norm\_hash} \\
	The performance of the normalized query hash is dependent on the number of filter arguments of the normalized query. In this evaluation it is constant, since in every test case, there are all filter arguments used. Hence, the evaluation needs the upper limit of the possible duration time. In the test cases the the duration is between 15 µs and 12061 µs with a median of 22 µs.  
	\item \textbf{result\_hash} \\
	The duration of the result hash creation is dependent on the length of the resulting file list. In the test cases the duration time of the result hash calculation is between 18 µs and 10692 µs with a median of 52 µs. 
	\todo{make analysis on result files}  
	\item \textbf{meta\_data} \\
	The meta data consists only on the number of result files. This is measured by the built-in len() operator in python, which has a static performance independent on the length of a list according to the official python wiki\footnote{https://wiki.python.org/moin/TimeComplexity}. In the test cases the meta data calculation takes a duration between 6 µs and 3583 µs with a median of 13 µs.
	\item \textbf{Check on existing query record} \\
	To ensure that there are no duplicate query entries in the database, the Query Handler checks if there is already an entry with the same normalized query hash and the same result hash. It depends on the amount of query entries in the database. The database query in the test cases takes between 1.488 ms and 169.772 ms with a median of 2.203 ms. 
	\todo{get reference on how fast a select statement is in PostGreSQL}  
	\item \textbf{Persisting new query record} \\
	The query\_pid is generated by the python uuid module, automatically by inserting the query entry into the database. Since the query entries are almost constant in size (see Section \ref{Evaluation:impact_stor}), the duration is constant. The duration of storing the query entry in the test cases is between 3.013 ms and 85.255 ms with a median of 5.183 ms.  
\end{itemize}
\todo{Graphik der dynamischen Sache erstellen / messen}
...
\begin{enumerate}
	...
	programming language / input data / code identifier / start and end time
	...
	\item \textbf{Query and persisting job entry} \\
	
	\todo{additional analysis}
\end{enumerate}
\todo{Performance der statischen Sachen messen}
\todo{Graphik der dynamischen Sache erstellen / messen}
...
\todo{Draw some plots and do basic statistics to answer (see enumeration):  Edzer wants to know how good your solution is – how would you prove that to him?}
\todo{Go into more detail at the query hash of the file list and the context result hash}
\begin{enumerate}
...
\todo{Appendix wieder hinzufügen}
%\begin{figure}[h]
%	\centering

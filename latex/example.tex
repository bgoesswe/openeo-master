...
\todo{change after discussion with Andi...}
...
\begin{enumerate}
	...
	\textit{BackendMonit} requires for the execution some linux packages. First the tool runs on Python 3.6, hence it has to be installed on the system. There are no additional python packages necessary for it. The version control system Git is a requirement on the back end, as every back end has its own GitHub repository, it will assumable be already installed. It needs the linux tools inotify and cron to be installed, which are installed by default in all debian based linux distributions and many others. 
	
	\item \textbf{Setup Configuration File} \\
	To make \textit{BackendMonit} adaptable for other systems a configuration file is introduced to describe its behavior. The tool comes with a default configuration file, which the back end provider can adapt to their architecture. The config file specifies all paths to the tools described in the previous point. Additionally, configuration file needs the path to the GitHub Repository used for the local back end repository.      
...
Back end developers manually update the OpenEO core API version of the EODC back end. Therefore, the data file containing this information is specified in the configuration file of \textit{BackendMonit}. 
...
\todo{describe how updates are managed at eodc}
...
\todo{implement version with time-stamp parameter}
...
\end{figure}
\todo{show process graph differences in the program and edit the image here}

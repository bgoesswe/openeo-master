...
The capturing of the job dependent data is limited to the job execution modules of the EODC back end. The EODC back end of the release version "0.0.2" transforms the process graph into separate docker containers. For every process in the process graph there is a docker container created that runs python code. The input of the process is the output of the previous process. The first process docker container has the original input data as input value. The result gets into a temporary folder separated by the different process results. Every process has its own temporary output directory until the whole process graph got executed, after that the temporary folders get deleted and only the result is saved and persisted.
To achieve the job provenance data capturing described in chapter \ref{Design:Job dependent provenance}, the processes have to provide information about the input, output data and the processing itself. Two different implementations were created to achieve the job dependent provenance capturing. The first implementation uses the python code wrapper noworkflow \cite{c9e0604becba42af96a9cb0a6f60018b}. Instead of calling the processing with python, it gets called by the noworkflow wrapper that automatically creates a meta-data database about the python execution. Since the wrapper is capturing the provenance in a very detailed way and highly influences the performance (see \ref{Evaluation:NvsP}), a second more simple implementation was done. This implementation adds the specific information of interest to the logging of the processing and reads the logging files to generate the context model. The second implementation is in plain python and is used for the evaluation of this thesis. As described in the chapter \ref{Design:Back end provenance} the programming language of the back end shall be used for the implementation of the capturing tool.  
\todo{Add Figure of the Job capturing component and the output hash component}
...
The context model gets persisted as a JSON file in the job execution meta-database. After the job is carried out in the EODC back end the results are saved in a folder named after the unique job identifier and the meta data information is stored in a PostGreSQL database. Jobs are saved in the Job table of the database and the context model is one column of this table. The context model JSON file gets created by the implementations described below. The context model has the following structure:
...
\begin{table}[]
	...
	\begin{tabular}{l|l}
		...
		\textbf{J5: Start and End time of process execution} & job\_id \\ \hline
...
\todo{Move to "additional implementation" and describe it a little bit less and add the problems or link to the evaluation}
...
\todo{Add image of the client visualisation --> remove it from thesis ?}
\todo{Landing Page...}

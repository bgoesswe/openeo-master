...
file_listA["input_files"]["state"] # Returns "EQUAL"
	...
	\end{listing}
	After executing the job the pidA is created as "qu-78aab90a-13e6-4722-9237-924713de6b43". The resulting file list consists of 51 files and the re-execution of the query results in the same 51 files. Therefore, the "state" of the re-execution contains the value "EQUAL".  
...
con.update_file()
	...
	\end{listing}
	The "update\_file" method is added to the python client to set the back end to simulate the update of the first file in the query result. The update sets the creation time-stamp of the file to the current date and time and renames the file with an additional "\_new". The addition happens right after the query execution. The update does not remove the old file from the result, but adds an additional file to the result files. 
...
file_listA["input_files"]["state"] # Returns "EQUAL"
	...
	\end{listing}
	The second call for the file list results in the same file list, since the old file is still available and the updated file is filtered out by the time-stamp of the original execution. The query contains the execution time-stamp of the original query and filters every file out of the result that has a creation time-stamp after that. 
	\todo{add query figure ?}
	\item \textbf{Run duplicate of Job A named Job B} \\
	\begin{listing}[ht]
		\begin{minted}{python}
...
(file_listA == file_listB) # Returns False
		\end{minted}
		\caption{Create job B, which uses the same process graph as Job A.}
		\label{lst:eva_datachange_4}
	\end{listing}
	The duplicate job B uses the same process graph as the job A, but gets executed after the update of the file takes place. Therefore, job B gets a new query PID and the resulting files differ, by the updated file. 
	\todo{show filelist, maybe in appendix...}
	\item \textbf{Run duplicate of Job A, by using the data PID of job A named Job C}\\
	\begin{listing}[ht]
		\begin{minted}{python}
...
(file_listA == file_listC) # Returns True
		\end{minted}
		\caption{Create job C, which uses the input data od pidA.}
		\label{lst:eva_datachange_5}
	\end{listing}
	job C is using the data identifier of job A (pidA) and uses therefore also the execution time-stamp of the original query execution. So job C uses the same data PID as jobA and the updated file is also filtered out of the resulting files. 
...
con.update_file(deleted=True)
	...
	\end{listing}
	The method above updates the first file of the query result and removes the original file at the back end. 
...
#'["\'date\': \'2017-05-24\', \'timestamp\': \'2018-08-11\', \'path\':
#\'/eodc/products/copernicus.eu/s2a_prd_msil1c/2017/05/24/S2A_MSIL1C_20170524T101031_N0205_R022_T32TQR_20170524T101353_new.zip\', \'name\': \'S2A_MSIL1C_20170524T101031_N0205_R022_T32TQR_20170524T101353_new\'}]: s2a_prd_msil1c <data.dependencies.arg_parser.BBox object at 0x7f90f9cbfe80> 2017-05-01T00:00:00Z 2017-05-31T23:59:59Z"]'
	\end{minted}
	\todo{show result as image}
	\caption{Re-execute pidA query after one file got updated and the old version got erased.}
	\label{lst:eva_datachange_7}
	\end{listing}	
	The re-execution of the query pidA results in a file list without the deleted file. Since files are filtered out by the query due to the execution time-stamp, the new file does not appear in the result file list. If the re-execution results not in the same file list, the "state" consists of a list of the newest files that replaced files that are no longer available. In this case the "state" contains only one file that replaces the original file. So users can see the alternatives for the original file, but not the original file itself. This is because the file list is not persisted in the query store, but the number of result files. If the number of result files is different at a re-execution, the query gets executed without the original execution timestamp and the result gets compared to the re-execution with the original execution timestamp. All files that are missing in the re-execution with the original execution timestamp are added to the "state" list.        
	\item \textbf{Run duplicate of Job A, by using the data PID of job A named Job D}\\
	\begin{listing}[ht]
	\begin{minted}{python}
# Take input data of job A by using the input data pid A of job A
...
pidD = jobD.get_data_pid()
	\end{minted}
	\caption{Run duplicate of Job A, by using the data PID of job A named Job D}
	\label{lst:eva_datachange_}
	\end{listing}

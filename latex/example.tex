...
\begin{table}[]
	...
	\begin{tabular}{l|l}
		...
		\textbf{\gls{cpu}} & Intel Core i5-6200U CPU @ 2.40GHz \\ 
		\textbf{\gls{gpu}} & NVIDIA GeForce 940MX (2GB DDR3)  \\ 
		\textbf{\gls{ram}} & 8 GB  \\ 
		\multicolumn{2}{c}{\textbf{Software}} \\ \hline
		\textbf{\gls{os}} & Ubuntu 18.04.1 LTS \\ 
...
This section evaluates special test cases to get an overview on how the solution behaves, if specific circumstances occur. The GitHub repository of the solution has unit tests in the "test" folder of the python client testing the basic functionality. The test cases in this section are created to look into the error proneness of the solution. The code as well as the results of the evaluation in this section are available on GitHub\footnote{https://github.com/bgoesswein/dataid\_openeo/tree/master/openeo-python-client/examples/}.
...
pidD = jobD.get_data_pid() # pidD != pidA
	...
	\end{code}
	Listing \ref{lst:eva_datachange_8} shows the code of running a new jobD with the query PID pidA like in Step 5. Step 6 deleted one file of the query PID from the back end. After the re-execution of the query, the back end notices that the number of result files is different than the original execution. It then looks for updated files like described in Step 7 and adds the most recent version of the missing file to the query result. This leads to a new query PID for jobD, since the query result has changed.  
\end{enumerate}


The test case shows how the solution behaves in update and deletion scenarios. The query PID gets updated on any changes of the result, even if it is used as input parameter for a new job. This leads to a consistent query PID. The researcher can see how the data has changed in the re-execution functionality. Note that the test case does not consist a case where a dataset got deleted without an updated dataset, since the behavior is the same as for the updated test case (see Step 6), except for an empty "state" attribute. 
...
\begin{table}[]
	...
	\begin{tabular}{r|r|r|r|r|r}
		...
		\textbf{Test} & \textbf{Ref. Back end} & \multicolumn{3}{c}{\textbf{Solution Back end}} &  \\ \textbf{Case} & \textbf{Total} & \textbf{Total} & \textbf{Query Handler} & \textbf{Context Model} & \textbf{Increase} \\ \hline
		1 & 800.136 ms & 831.136 ms & 14.318 ms & 16.682 ms & 3.87 \% \\ 
		2 & 284.361 ms & 335.556 ms & 24.167 ms & 27.028 ms & 18.00 \% \\ 
		3 & 307.439 ms & 359.268 ms & 22.951 ms & 28.878 ms & 16.86 \% \\ 
		4 & 266.351 ms & 309.541 ms & 20.351 ms & 22.838 ms & 16.22 \% \\ 
		5 & 284.324 ms & 345.757 ms & 26.081 ms & 35.351 ms & 21.61 \% \\ 
		6 & 289.622 ms & 347.578 ms & 24.956 ms & 33.000 ms & 20.01 \% \\ 
		7 & 270.889 ms & 327.289 ms & 25.533 ms & 30.867 ms & 20.82 \% \\ 
		8 & 278.091 ms & 329.409 ms & 22.909 ms & 28.409 ms & 18.45 \% \\ 
		9 & 13296.605 ms & 13604.698 ms & 259.884 ms & 48.209 ms & 2.32 \% \\ 
		10 & 386.684 ms & 435.895 ms & 22.289 ms & 26.921 ms & 12.73 \% \\ 
		11 & 40512.538 ms & 40770.026 ms & 212.667 ms & 44.821 ms & 0.64 \% \\ 
		12 & 528.450 ms & 566.925 ms & 18.675 ms & 19.800 ms & 7.28 \% \\ 
		13 & 1785.462 ms & 1818.641 ms & 18.410 ms & 14.769 ms & 1.86 \% \\ 
		14 & 40461.526 ms & 40729.368 ms & 223.000 ms & 44.842 ms & 0.66 \% \\ 
		15 & 422.512 ms & 466.854 ms & 20.146 ms & 24.195 ms & 10.49 \% \\ 
		16 & 44672.378 ms & 44940.022 ms & 221.889 ms & 45.756 ms & 0.60 \% \\ 
		17 & 753.256 ms & 787.436 ms & 15.897 ms & 18.282 ms & 4.54 \% \\ 
		18 & 281.611 ms & 344.389 ms & 26.278 ms & 36.500 ms & 22.29 \% \\ \hline
		\textbf{All} & \textbf{8104.569 ms} & \textbf{8202.766 ms} & \textbf{67.800 ms} & \textbf{30.397 ms} & \textbf{11.07
		 \%} \\ 
	\end{tabular}
	\label{Tab:eva_performance}
\end{table}

Table \ref{Tab:eva_performance} shows the mean duration time of 50 runs of the test case execution. The second column presents the duration of the reference back end, the other columns are measurements of the solution system. Including to the total execution duration of the solution, the duration of the Query Handler and the Context Model creation is measured and presented in the next two columns. The last column consists of the relative increase in duration of the solution back end compared to the solution back end. The last row of the table shows the mean duration of all test cases.
The execution time of the reference back end is directly depending on the spatial and temporal input of the test case. It represents the complexity of the test case processing, since the back end is only processing the data. The increase in execution duration between the systems is due to the additional functionality of the Query Handler and the Context Model creation. The performance of the Query Handler is similar between the test runs, except for the more complex test cases. The increase of execution time is due to the bigger query result set and the calculation of the SHA-256 hash of it. The other parts of the Query record are not depending on the size of the query result, but on the original query. The original query is not changing in terms of size, because only the filter arguments change their values. The Context Model creation performance is not affected much by the complexity of the test cases. The captured data for the context model is not related to the complexity of the execution. The "Increase" column of Table \ref{Tab:eva_performance} shows that simple test cases are affected the most in terms of relative performance loss, because the Query Handler and the Context Model have elements that need the same duration time independent of the test case configuration.   
...
\begin{listing}[ht]
	\begin{minted}{sql}
-- Context Model 
SELECT sum(pg_column_size(metrics)) as filesize, count(*) as filerow 
...
\begin{table}[]
	...
	\begin{tabular}{r|r|r|r|r}
	 ...
	 \textbf{Test Case} & \textbf{Context Model} & \textbf{Query} & \textbf{QueryJob} & \textbf{Sum} \\ \hline
...
Table \ref{Tab:eva_storage} shows the result of the queries. It contains the mean storage of the 50 runs per test case and the average storage size of all test cases in the last row. The column "Context Model" is only the size of the column "metrics" in the Job table, since the context model is stored in the column and the remaining Job table is not modified by the solution. The "Query" and "QueryJob" column show the storage needed for the Query or QueryJob table data record. The "Sum" column is calculated by summing up the Context Model, Query table and QueryJob table storage. It represents the additional storage needed by the solution compared to the reference back end. There is no element in the context model that can vary in size, so that needed size is statically at 1.043 kByte. The same occurs at the QueryJob table, which just maps the Query and Job table and contains only the identifier and timestamps for creation and modification. The Query data records has a slightly varying size, because of the string length of the parameters in the original query. The remaining parts of the Query table are static in storage usage. The original query makes up for the most size needed in the Query table (e.g. 959 Bytes of 1521.432 Bytes in Testcase 18), because of the XML annotations. In summary the average additional storage needed by the reference back end per Job with a new Query entry is 2.677 kBytes. If the used query is already in the Query table, only an additional 1.043 kBytes of storage is needed by the solution. 
